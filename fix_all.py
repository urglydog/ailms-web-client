...
# 1. Remove courseResourcesApi, UploadStaticMaterialModal, unused icons
content = content.replace("import { courseResourcesApi } from '@/lib/api/courseResourcesApi';", "")
content = content.replace("import { UploadStaticMaterialModal } from './UploadStaticMaterialModal';", "")
content = content.replace("Folder, FileText, MoreVertical, Plus, Trash2, Edit2, Play, BookOpen, Layers", "Folder, FileText, MoreVertical, Plus, Trash2, BookOpen, Layers")

# 2. Remove unused state variables
content = content.replace("const [currentFolderId, setCurrentFolderId] = useState<number | null>(null);", "")
content = content.replace("const [breadcrumbs, setBreadcrumbs] = useState", "const [breadcrumbs] = useState")
content = content.replace("const [showUploadStatic, setShowUploadStatic] = useState(false);", "")

# 3. Fix TS any type on folders
content = content.replace("const folders: any[] = [];", "const folders: {id: number, name: string}[] = [];")
content = content.replace("folders.map((f: any) => (", "folders.map((f) => (")

# 4. Remove unused mutations
content = re.sub(
    r"  const toggleMindmapMutation[\s\S]*?}\);\n",
    "",
    content
)
content = re.sub(
    r"  const toggleFlashcardMutation[\s\S]*?}\);\n",
    "",
    content
)
content = re.sub(
    r"  const toggleQuizOfficialMutation[\s\S]*?}\);\n",
    "",
    content
)

# 5. Remove unused 'mat' in renderDeleteModal
content = content.replace("const mat = materials?.find(m => m.id === confirmDeleteId);", "")


# 6. Apply UI Decoupling (from fix_ui_v2.py)
...
# Remove selectedTarget state safely
content = re.sub(
