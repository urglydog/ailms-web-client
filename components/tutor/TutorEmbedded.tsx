...
interface TutorEmbeddedProps {
  courseId: number;
  lessonId: number;
  /** `contextLessonId` = bài học câu trả lời đó thực sự nói tới (thường trùng bài đang mở, có
...
/**
 * UC30 — Gia sư AI Socratic, NHÚNG làm nội dung 1 tab trong sidebar bài học (giao diện tham khảo
 * Udemy — tab "AI Assistant" đứng cạnh "Course content", không che video nữa).
 *
 * BUG THẬT (06/09/2026): trước đây đây là `TutorPanel` — panel TRƯỢT TỪ CẠNH PHẢI, che lên trên
 * video khi mở (`fixed inset-y-0 right-0` + backdrop + `isOpen`/`onClose`). Đổi sang tab nhúng
 * theo đúng ảnh mẫu Udemy bạn gửi — bỏ hẳn phần "khung nổi" (backdrop, dịch chuyển ngang, nút ✕),
 * giữ NGUYÊN VẸN phần lõi (lịch sử hội thoại, gửi câu hỏi, đính kèm tệp, mốc thời gian) không đổi
 * hành vi gì cả — chỉ đổi "cái khung bọc quanh nó". Cao đúng bằng chiều cao khối cha (`h-full`) do
 * trang cha (`learn/[lessonId]/page.tsx`) cấp, không tự set `fixed`/chiều cao tuyệt đối nữa.
 *
 * BUG THẬT (06/09/2026 — sửa lần 2): danh sách lịch sử trò chuyện giờ dùng CHUNG cho cả khóa học
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b border-line bg-accent px-4 py-3 text-white">
        <div>
          <h3 className="font-display text-sm font-semibold">Gia sư AI Socratic</h3>
          <p className="text-[11px] opacity-90">Hỏi về nội dung bài học, không cho đáp án trực tiếp</p>
        </div>
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`flex flex-col ${msg.sender === 'USER' ? 'items-end' : 'items-start'}`}>
                  {msg.attachments.length > 0 && (
                    <div className="mb-1 flex max-w-[90%] flex-wrap justify-end gap-1.5">
