            ...
            {isLoggedIn ? (
              <div className="relative ml-1" ref={accountMenuRef}>
                <button
                  ...
                  className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-accent font-display text-[13px] font-bold text-white uppercase"
                  title={userInfo?.name}
                >
                  {userInfo?.name ? userInfo.name.charAt(0) : 'U'}
                </button>
                {accountMenuOpen && (
                  <div className="absolute right-0 top-12 z-[200] flex w-56 flex-col rounded-2xl border border-line bg-white p-2 shadow-[0_20px_50px_rgba(19,22,32,0.15)]">
                    <div className="px-3 py-2 border-b border-line-soft mb-1">
                      <p className="text-sm font-bold text-ink truncate">{userInfo?.name || 'Người dùng'}</p>
                      <p className="text-xs font-medium text-ink-muted uppercase">{userInfo?.role || 'Học viên'}</p>
                    </div>
